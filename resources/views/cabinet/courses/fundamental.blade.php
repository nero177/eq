...
@section('content')
    <main class="cabinet">
        <div class="content wrapper">
            ...
            <div class="cabinet-subtitle">{!! __('site.fundamental') !!}</div>
        </div>
        <section class="lessons-grid content wrapper">
          <div class="grid-container">
              @foreach ($lessons as $lesson)
                  <a class="item item-fundamental aos-init aos-animate" data-aos="fade-up"
                      data-aos-anchor-placement="top-bottom" data-aos-duration="800"
                      href="{{ localize_url('/cabinet/lesson/' . $lesson->id) }}">
                      <img src="{{ $lesson->getFirstMediaUrl('thumbnail') }}" alt="lesson" />
